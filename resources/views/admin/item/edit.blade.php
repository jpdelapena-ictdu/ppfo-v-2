                ...
                <select class="form-control js-single" name="building" id="building">
                  <option value="none">---Select a Building---</option>
                   ...
                   @foreach($buildings as $row)
                       <option value="{{ $row->id }}" @foreach($rooms as $room) @if($item->room_id == $room->id)@if($room->building_id == $row->id)  selected @endif @endif @endforeach>{{ '(' .$row->short_name. ') ' .$row->name }}</option>
                  @endforeach
                </select>
              </div>
