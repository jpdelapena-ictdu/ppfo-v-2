...
{{--                     <a href="#" class="btn btn-default btn-xs" id="viewItem" data-toggle="modal" data-target="#messageModal{{$row->id}}"><i class="fa fa-eye"></i> View</a>  --}}
                    @if($row->status == 0)
                    <a href="{{ route('admin.order.approve', $row->id) }}" class="btn btn-success btn-xs"><i class="fa fa-edit"></i> Approve</a>
                    <a href="{{ route('admin.order.reject', $row->id) }}" class="btn btn-danger btn-xs"><i class="fa fa-edit"></i> Reject</a>
                    @elseif($row->status == 1)
                    <a href="{{ route('admin.order.reject', $row->id) }}" class="btn btn-danger btn-xs"><i class="fa fa-edit"></i> Reject</a>
                    @elseif($row->status == 2)
                    <a href="{{ route('admin.order.approve', $row->id) }}" class="btn btn-success btn-xs"><i class="fa fa-edit"></i> Approve</a>
